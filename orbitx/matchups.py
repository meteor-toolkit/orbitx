...
class Matchups:
    ...
    def from_netcdf(
        ...
        reference_date = np.array([matchups_xarray["reference_date"].values], dtype = "datetime64[s]")[0]
        time_datetime = np.array(matchups_xarray[f"time_datetime"].values, dtype = "datetime64[s]")
        ...
        start_date = sec_since_to_datetime64(matchups_xarray.attrs["start_date"], reference_date=reference_date)
        end_date = sec_since_to_datetime64(matchups_xarray.attrs["end_date"], reference_date=reference_date)
        propagation_sampling_interval = np.array([matchups_xarray.attrs["propagation_sampling_interval"]], dtype="timedelta64[s]")[0]
        interpolation_sampling_interval = np.array([matchups_xarray.attrs["interpolation_sampling_interval"]], dtype="timedelta64[s]")[0]
        time_diff_threshold = np.array([matchups_xarray.attrs["time_diff_threshold"]], dtype="timedelta64[s]")[0]
        ...
        for sat_index, _ in enumerate(satellites[1:]):
            time_datetime_sat = np.array(matchups_xarray[f"time_datetime{sat_index + 2}"].values, dtype = "datetime64[s]")
            time_sat = np.array([datetime64_to_sec_since(date, reference_date) for date in time_datetime_sat], dtype = float)
            matchups_xarray = matchups_xarray.assign(
                variables={
    ...
    def to_netcdf(
        ...
        sampling_part = (
            f"psi{self.orbit.propagation_sampling_interval.item().total_seconds()}_isi{self.orbit.interpolation_sampling_interval.item().total_seconds()}"
        )
    ...
    def plot(
        ...
        delays = [[delay.item().total_seconds() for delay in delays_sat] for delays_sat in delays]
