...
class TestGetDelay(unittest.TestCase):
    def test_get_delay_1_existing(self):
        existing_orbits = xr.Dataset(
            ...
            coords={"matchup_index": np.arange(3), "satellite": [""]},
        )
        ...
        np.array_equal(
            ...
            np.array([0, -5, 5], dtype="timedelta64[s]"),
        )
    ...
    def test_get_delay_2_existing(self):
        existing_orbits = xr.Dataset(
            ...
            coords={"matchup_index": np.arange(3), "satellite": ["0", "1"]},
        )
        ...
        new_orbit = xr.Dataset(
            ...
            coords={"matchup_index": np.arange(3), "satellite": ["2"]},
        )

        np.array_equal(
            get_delay(existing_orbits, new_orbit),
            np.array([[0, 0], [-5, -5], [5, 5]], dtype="timedelta64[s]"),
        )
