...
class TestIsLand(unittest.TestCase):
    def test_is_land_capitals(self):
        ...
        self.assertTrue(np.all(is_land(capital_list["Longitude"], capital_list["Latitude"])))
    ...
    def test_is_land_seas(self):
        ...
        self.assertTrue(np.all(np.logical_not(is_land(sea_list["Longitude"], sea_list["Latitude"]))))
