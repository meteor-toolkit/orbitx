...
class MatchUp:
    ...
    @staticmethod
    def roi_indices(
        orbits: dict, latmin, latmax, lonmin, lonmax, interpolation_sampling_interval
    ):
        """
    ...
    def sat_to_sat_matchup(
        ...
        if len(sat1_jump_start_idx) <= len(sat2_jump_start_idx):
            for i in range(len(sat1_jump_start_idx)):
                # timevec1 = []
                timevec1 = sat1_inROI_time[
                    sat1_jump_start_idx[i] : sat1_jump_stop_idx[i]
                ]
                ...
                for j in range(len(sat2_jump_start_idx)):
                    ...
                    if (
                        np.abs(np.mean(timevec2) - np.mean(timevec1))
                        <= time_diff_threshold
                    ):
    ...
    def search(
        ...
        (
            ...
            sat_inROI_lat,
            sat_inROI_lon,
            sat_jump_start_index,
            sat_jump_stop_index,
        ) = self.roi_indices(
            orbits, latmin, latmax, lonmin, lonmax, interpolation_sampling_interval
        )

        if len(sat_inROI_time) == 2:
            match_event_idx = self.sat_to_sat_matchup(
                sat_jump_start_index[0],
                sat_jump_stop_index[0],
                sat_inROI_time[0],
                sat_jump_start_index[1],
                sat_jump_stop_index[1],
                sat_inROI_time[1],
                time_diff_threshold,
            )
            ...
            sat_match_event_idx = [
                [i[0] for i in match_event_idx],
                [i[1] for i in match_event_idx],
            ]
            sat_to_sat_tmpdist = [i[2] for i in match_event_idx]

        elif len(sat_inROI_time) > 2 and not isinstance(
            sat_inROI_time[0], (float, int)
        ):
            # figure out how to run for multiple orbits
            sat_match_event_idx = []
            sat_to_sat_tmpdist = (
                []
            )  # could be the greatest temporal distance between any pair of satellites
            pass
        else:
        ...
        sat_inROI_datetime = [
            [datetime.datetime(2000, 1, 1) + datetime.timedelta(seconds=i) for i in j]
            for j in sat_inROI_time
        ]
        sat_starttime = [
            [
                sat_inROI_datetime[i][sat_jump_start_index[i][k]]
                for k in sat_match_event_idx[i]
            ]
            for i in range(sat_number)
        ]
        sat_stoptime = [
            [
                sat_inROI_datetime[i][sat_jump_stop_index[i][k]]
                for k in sat_match_event_idx[i]
            ]
            for i in range(sat_number)
        ]
        ...
        sat_startlat = [
            [
                sat_inROI_lat[i][sat_jump_start_index[i][j]]
                for j in sat_match_event_idx[i]
            ]
            for i, k in enumerate(orbits.keys())
        ]
        sat_stoplat = [
            [
                sat_inROI_lat[i][sat_jump_stop_index[i][j]]
                for j in sat_match_event_idx[i]
            ]
            for i, k in enumerate(orbits.keys())
        ]
        sat_startlon = [
            [
                sat_inROI_lon[i][sat_jump_start_index[i][j]]
                for j in sat_match_event_idx[i]
            ]
            for i, k in enumerate(orbits.keys())
        ]
        sat_stoplon = [
            [
                sat_inROI_lon[i][sat_jump_stop_index[i][j]]
                for j in sat_match_event_idx[i]
            ]
            for i, k in enumerate(orbits.keys())
        ]
    ...
    def save(matchup_info, path, fname):
        ...
        with open(os.path.join(path, fname), "w") as file:
            ...
            for x in zip(*matchup_info):
                file.write(
                    "},{".join([str(i) for i in range(len(first_line.split(",")))])
                    .join(["{", "}\n"])
                    .format(*x)
                )
