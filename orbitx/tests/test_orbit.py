...
class TestORBIT(unittest.TestCase):
    ...
    def test_form_sample_space(self):
        ...
        exp_smpl_space = [datetime.datetime(2000, 1, 1, 0, 0),datetime.datetime(2000, 1, 1, 12, 0),datetime.datetime(2000, 1, 2, 0, 0)]
        exp_smpl_space_secs_since_2000 = np.array([0., 43200., 86400.])

        smpl_space, smpl_space_secs_since_2000 = Orbit.form_sample_space(start_time, end_time, prop_smpl_interval)

        self.assertEqual(exp_smpl_space, smpl_space)
        self.assertTrue((exp_smpl_space_secs_since_2000 == smpl_space_secs_since_2000).all())
    ...
    def test_get_matching_indices(self):
        ...
        sim_time = [1,1.5,2,3,4.2,6.7,8,9,10,78] # These are the simulation (propagation) time stamps

        exp_idx_tle = [0, 1, 2, 3, 5] # These are the indices of the corresponding time stamps in TLE time vector
        exp_idx_sim = [0, 4, 6, 8, 9] # These are the indices of the corresponding time stamps in simulation vector

        idx_sim, idx_tle = Orbit.get_matching_indices(np.array(sim_time), np.array(tle_time))
    ...
    def test_simulate_orbit(self):
        ...
        S6_start_time = datetime.timedelta(seconds=exp_time[0]) + datetime.datetime(2000, 1, 1, 0, 0, 0)
        S6_end_time = datetime.timedelta(seconds=exp_time[len(exp_time)-1]) + datetime.datetime(2000, 1, 1, 0, 0, 0)
        ...
        distance = [cal_dist_d2m(lat[i], lon[i], exp_lat[i], exp_lon[i]) for i in range(len(lat))]
    ...
    def test_interpolate_orbit(self):
        ...
        lat, lon, time = orbit.interpolate_orbit(np.array(sat_sec_since), np.array(sat_lat_sim), np.array(sat_lon_sim), interpolation_sampling_interval)
