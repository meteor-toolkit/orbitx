...
def return_matchups(
    ...
    # initialise satellite tle info objects
    tle_1 = TLE(sat_1, start_time, end_time)
    tle_2 = TLE(sat_2, start_time, end_time)
    # set tle lines info
    tle_1.set_tle()
    tle_2.set_tle()
    ...
    #
    # sat1_smpl_breakup_idx, tle1_ref_lines = breakup_smpl_space(smpl_space_secs_since_2000, tle_1.seconds_since_2000)
    # sat2_smpl_breakup_idx, tle2_ref_lines = breakup_smpl_space(smpl_space_secs_since_2000, tle_2.seconds_since_2000)
    #
    # # endregion

    # EVERYTHING BELOW HERE - SIMULATION CLASS
    # what are the inputs?
    # tle_line1, tle_line2, sampling_space, prop
    ...
    #     secsince1, lat1, lon1, alt1, el1, az1 = propagate_orbit(tle_1.line1[i], tle_1.line2[i],
    ...
    #     secsince2, lat2, lon2, alt2, el2, az2 = propagate_orbit(tle_2.line1[i], tle_2.line2[i],
    ...
    sat1_lat_sim, sat1_lon_sim, sat1_secsince = sim_orbit(
        tle_1, propagation_sampling_interval
    )
    sat2_lat_sim, sat2_lon_sim, sat2_secsince = sim_orbit(
        tle_2, propagation_sampling_interval
    )
    ...
    fname1 = (
        ...
        + str(end_time)[8:10]
        + "_samplinginterval"
        + str(propagation_sampling_interval)
        + ".txt"
    )
    fname2 = (
        sat_2
    ...
    sat1_inROI_idx = (
        (sat1_lat_sim_interp > latmin)
        & (sat1_lat_sim_interp < latmax)
        & (sat1_lon_sim_interp > lonmin)
        & (sat1_lon_sim_interp < lonmax)
    )
    sat2_inROI_idx = (
        (sat2_lat_sim_interp > latmin)
        & (sat2_lat_sim_interp < latmax)
        & (sat2_lon_sim_interp > lonmin)
        & (sat2_lon_sim_interp < lonmax)
    )
    ...
    sat2_time_dif = (
        sat2_inROI_time[1 : len(sat2_inROI_time)]
        - sat2_inROI_time[0 : len(sat2_inROI_time) - 1]
    )
    ...
    sat1_inROI_datetime = [
        ...
        for i in sat1_inROI_time
    ]
    sat2_inROI_datetime = [
        datetime.datetime(2000, 1, 1, 0, 0, 0) + datetime.timedelta(seconds=i)
        for i in sat2_inROI_time
    ]

    sat1_starttime = [
        sat1_inROI_datetime[sat1_jump_start_idx[i]] for i in sat1_match_event_idx
    ]
    sat1_stoptime = [
        sat1_inROI_datetime[sat1_jump_stop_idx[i]] for i in sat1_match_event_idx
    ]
    sat2_starttime = [
        sat2_inROI_datetime[sat2_jump_start_idx[i]] for i in sat2_match_event_idx
    ]
    sat2_stoptime = [
        sat2_inROI_datetime[sat2_jump_stop_idx[i]] for i in sat2_match_event_idx
    ]

    sat1_startlat = [
        sat1_inROI_lat[sat1_jump_start_idx[i]] for i in sat1_match_event_idx
    ]
    sat1_stoplat = [sat1_inROI_lat[sat1_jump_stop_idx[i]] for i in sat1_match_event_idx]
    sat2_startlat = [
        sat2_inROI_lat[sat2_jump_start_idx[i]] for i in sat2_match_event_idx
    ]
    sat2_stoplat = [sat2_inROI_lat[sat2_jump_stop_idx[i]] for i in sat2_match_event_idx]

    sat1_startlon = [
        sat1_inROI_lon[sat1_jump_start_idx[i]] for i in sat1_match_event_idx
    ]
    sat1_stoplon = [sat1_inROI_lon[sat1_jump_stop_idx[i]] for i in sat1_match_event_idx]
    sat2_startlon = [
        sat2_inROI_lon[sat2_jump_start_idx[i]] for i in sat2_match_event_idx
    ]
    ...
    with open(os.path.join(output_path_matchups, fname), "w") as file:
        file.write(
            "sat1, starttime, stoptime, startlat, stoplat, startlon, stoplon, sat2, starttime, stoptime, startlat, stoplat, startlon, stoplon, temporal_distance\n"
        )
...
def sim_orbit(tle, propagation_sampling_interval):
    """
    Return latitude, longitude and time arrays for full simulated orbit
    """
    smpl_space, smpl_space_secs_since_2000 = form_smpl_space(
        tle.start_time, tle.end_time, propagation_sampling_interval
    )
    sat_smpl_breakup_idx, tle_ref_lines = breakup_smpl_space(
        smpl_space_secs_since_2000, tle.seconds_since_2000
    )
    sat_lat_sim = []
    sat_lon_sim = []
    sat_sec_since = []
    for i in range(len(tle_ref_lines) - 1):
        ...
        secsince1, lat1, lon1, alt1, el1, az1 = propagate_orbit(
            tle.line1[tle_ref_lines[i]],
            tle.line2[tle_ref_lines[i]],
