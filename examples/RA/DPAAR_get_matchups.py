...
for year in range(2022, 2025):
    ...
    for month_number in range(1, 2):
        ...
        if (year == 2022 and month_number in [1, 2, 3, 4]) or (
            year == 2025 and month_number in [1, 7, 8, 9, 10, 11, 12]
        ):
            continue
        ...
        end_time_input = datetime.datetime(
            year, month_number, month_end_day, 23, 59, 59
        )
        ...
        try:
            ...
            print(
                f"Saving file with start date {start_time_input} and end date {end_time_input}"
            )
            ...
            # Save plot
            fig.savefig(
                os.path.join("results", "S6_J3", f"{filename[:-3]}_plot.png"),
                dpi=300,
                bbox_inches="tight",
            )
            ...
            logging.error(
                f"An error occurred for start date {start_time_input} and end date {end_time_input}",
                exc_info=True,
